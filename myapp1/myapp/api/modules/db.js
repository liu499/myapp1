...
//多表联查
module.exports.getaddShopList=function(whereObj,cb){
    _connect(function(db){
        db.collection("addproduct").aggregate([
            {$match:whereObj},//查询
            {
                $lookup:{
                    from:"productList",// 你要和哪个集合进行合并
                    localField:"_id",// 本集合当中连接的依据字段
                    foreignField:"shopTypeId",// 外部的集合连接的依据字段
...
//商品列表与商品联查
module.exports.getShopManageList=function(skip,limit,cb){
    _connect(function(db){
        db.collection("productList").aggregate([
            {$skip:skip},
            {$limit:limit},
            {
                $lookup:{
                    from:"addproduct",// 你要和哪个集合进行合并
                    localField:"shopTypeId",// 本集合当中连接的依据字段
                    foreignField:"_id",// 外部的集合连接的依据字段
                    as: 'typeInfo'// 查找到的信息放到该字段 当中
                }
        ...
        db.collection("adminLog").aggregate([
            ...
            },
			{
				$lookup:{
					...
					foreignField:"logType",// 外部的集合连接的依据字段
